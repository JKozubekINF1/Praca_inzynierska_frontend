...
import type { 
    AdminUser, AdminAnnouncement, SystemLog, DashboardStats, TabType, ModalState, PagedResult 
} from '../types/admin';
...
export const useAdminLogic = (activeTab: TabType) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            if (activeTab === 'dashboard') {
                ...
                const res = await axios.get<PagedResult<AdminUser>>(`${API_BASE_URL}/api/admin/users${queryParams}`, config);
                setData(prev => ({ ...prev, users: res.data.items }));
                setTotalPages(res.data.totalPages);
            } 
            else if (activeTab === 'announcements') {
                const res = await axios.get<PagedResult<AdminAnnouncement>>(`${API_BASE_URL}/api/admin/announcements${queryParams}`, config);
    ...
    return {
        data, loading, modal, setModal, openModal, executeAction, navigate,
        page, setPage, totalPages, search, setSearch
    };
};