...
import { PREDEFINED_FEATURES } from '../constants/index'; 
...
const EditAnnouncementPage: React.FC = () => {
    ...
    const {
        category, setCategory, baseData, setBaseData, vehicleData, setVehicleData, partData, setPartData, 
        photos, features, setFeatures,
        loading, globalError, fieldErrors,
        handleBaseChange, handleVehicleChange, handlePartChange, 
        handlePhotosChange, removePhoto, toggleFeature, submitEdit
    } = useAnnouncementForm();
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-10 px-4">
            <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-xl p-8 border border-gray-200">
                <h1 className="text-3xl font-extrabold text-black mb-8 text-center">Edytuj ogłoszenie</h1>
                ...
                <form onSubmit={(e) => submitEdit(e, Number(id))} className="space-y-6">
                    ...
                    <div className="bg-yellow-50 p-4 rounded border border-yellow-200">
                        <p className="text-sm text-yellow-800 font-bold">Uwaga: Jeśli dodasz tutaj nowe zdjęcia, zastąpią one wszystkie poprzednie zdjęcia tego ogłoszenia.</p>
                    </div>
                    ...
                    {category === "Pojazd" && (
                        <div className="mt-6 border-t pt-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div><label className={labelClass}>Marka</label><input name="brand" value={vehicleData.brand} onChange={handleVehicleChange} className={inputClass} /></div>
                                <div><label className={labelClass}>Model</label><input name="model" value={vehicleData.model} onChange={handleVehicleChange} className={inputClass} /></div>
                                <div><label className={labelClass}>Generacja</label><input name="generation" value={vehicleData.generation} onChange={handleVehicleChange} className={inputClass} /></div>
                                <div><label className={labelClass}>Rok</label><input type="number" name="year" value={vehicleData.year} onChange={handleVehicleChange} className={inputClass} /></div>
                                <div><label className={labelClass}>Przebieg</label><input type="number" name="mileage" value={vehicleData.mileage} onChange={handleVehicleChange} className={inputClass} /></div>
                                <div><label className={labelClass}>Moc (KM)</label><input type="number" name="enginePower" value={vehicleData.enginePower} onChange={handleVehicleChange} className={inputClass} /></div>
                                <div><label className={labelClass}>Pojemność</label><input type="number" name="engineCapacity" value={vehicleData.engineCapacity} onChange={handleVehicleChange} className={inputClass} /></div>
                                <div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-gray-50 p-4 rounded-lg">
                                    {PREDEFINED_FEATURES.map(f => (
                                        <label key={f} className="flex items-center space-x-2 cursor-pointer">
                                            <input type="checkbox" checked={features.includes(f)} onChange={() => toggleFeature(f)} className="rounded w-5 h-5" />
                                            <span>{f}</span>
                                        </label>
                                    ))}
                                </div>
                    ...
                    <div className="space-y-4 border-t pt-6 mt-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" value={baseData.description} onChange={handleBaseChange} className={`${inputClass} h-40`} />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className={labelClass}>Telefon</label><input name="phoneNumber" value={baseData.phoneNumber} onChange={handleBaseChange} className={inputClass} /></div>
                            <div>
                    ...
                    <div className="pt-8 pb-4">
                        <button type="submit" disabled={loading} className="w-full bg-orange-600 text-white font-bold py-4 rounded-lg hover:bg-orange-700 disabled:opacity-50 transition shadow-lg text-lg">
