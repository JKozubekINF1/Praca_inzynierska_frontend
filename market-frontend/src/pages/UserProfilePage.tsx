...
const UserProfilePage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const res = await axios.get(`${API_BASE_URL}/api/admin/users/${id}`, { withCredentials: true });
                setUser(res.data);
            } catch (error) {
            ...
            <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
                <div className={`p-6 text-white ${user.isBanned ? 'bg-red-600' : 'bg-blue-600'}`}>
                    <div className="flex justify-between items-center">
                        ...
                        <div className="text-right">
                            ...
                            {user.isBanned && <div className="mt-2 font-bold bg-white text-red-600 px-2 py-1 rounded text-xs text-center">ZBANOWANY</div>}
                        </div>
                    </div>
                </div>
                <div className="p-6 border-b">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
                        ...
                        <p><strong>Telefon:</strong> {user.phoneNumber || 'Brak'}</p>
                        <p><strong>Imię:</strong> {user.name || 'Brak'}</p>
                        <p><strong>Nazwisko:</strong> {user.surname || 'Brak'}</p>
                    </div>
                </div>
                <div className="p-6">
                    <h2 className="text-xl font-bold mb-4">Ogłoszenia użytkownika ({user.announcements?.length || 0})</h2>
                    {user.announcements && user.announcements.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {user.announcements.map((ad: any) => (
                                <Link to={`/announcements/${ad.id}`} key={ad.id} className="block border rounded-lg hover:shadow-md transition p-3">
                                    <div className="flex items-center space-x-3">
                                        {ad.photoUrl ? (
                                            <img src={`${API_BASE_URL}/${ad.photoUrl}`} alt={ad.title} className="w-16 h-16 object-cover rounded" />
                                        ) : (
                                            <div className="w-16 h-16 bg-gray-200 rounded flex items-center justify-center text-gray-400 text-xs">Brak foto</div>
                                        )}
                                        <div>
