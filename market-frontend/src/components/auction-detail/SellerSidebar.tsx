...
export const SellerSidebar: React.FC<Props> = ({ announcement }) => {
    return (
        <div className="sticky top-6 space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                <h1 className="text-2xl font-bold text-gray-900 mb-2 leading-tight">{announcement.title}</h1>
                <div className="text-sm text-gray-500 mb-6">
                    {announcement.vehicleDetails?.year ? `${announcement.vehicleDetails.year} • ` : ''} {announcement.category}
                </div>
                
                <div className="text-4xl font-extrabold text-gray-900 mb-6 tracking-tight">
                    {announcement.price.toLocaleString('pl-PL')} <span className="text-xl font-medium text-gray-500">PLN</span>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <button className="w-full bg-white border-2 border-gray-200 text-gray-700 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700 font-bold py-4 rounded-xl transition flex items-center justify-center gap-2">
                        Napisz wiadomość
                    </button>
                </div>
            </div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="font-bold text-gray-900 text-lg">{announcement.user?.username || 'Użytkownik'}</p>
